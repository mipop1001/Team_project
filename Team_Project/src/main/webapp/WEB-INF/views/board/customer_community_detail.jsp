...
<body>
	<div class="body_warp">
		<div class="body_main">
			<div class="body_big_title">캠핑경험담</div>
			<div class="body_info">
				<div class="body_wirter_date_place_title_content_grade">
					<div class="body_wirter_date_place">
						<c:forEach items="${list }" var="community_detail">
							<input type="hidden"
								value="${community_detail.community_board_number }"
								id="board_number">
					...
					${a.member_id }
							...
							<div class="body_date">
								${community_detail.community_board_date }</div>
							<div class="body_place">
								${community_detail.community_board_location }</div>
						</c:forEach>
					</div>
					...
					<div class="body_content_grade">
						<c:forEach items="${list }" var="community_detail">
							<div class="body_grade">
								${community_detail.community_board_grade }</div>
							<div class="body_content">
								${community_detail.community_board_content }</div>
						</c:forEach>
					</div>
				</div>
				<div class="body_detail_image">
					<c:forEach items="${list }" var="community_detail">
						<div class="image_one">
							<img
								src="product_sum_image/${community_detail.community_board_sum_image }">
							<img
								src="product_intro_image/${community_detail.community_board_detail_image1 }">
							<img
								src="product_intro_image/${community_detail.community_board_detail_image2 }">
							<img
								src="product_intro_image/${community_detail.community_board_detail_image3 }">
						</div>
					</c:forEach>
				</div>
				<div class="re_write">
					<div id="commentList">
						<c:forEach items="${list2}" var="comment">
							<div>
								<hr>
								작성 시간: ${comment.comment_date} <br> 작성자:
								${comment.member_id} <br> 내용: ${comment.comment_text} <br>
								<a href="javascript:void(0);"
									onclick="deleteComment('${comment.member_id}', '${comment.comment_text}', '${comment.comment_number}')">삭제</a>
								<hr>
							</div>
						</c:forEach>
					</div>
					<c:choose>
						<c:when test="${loginstatus == true }">
							<h4>댓글</h4>
							<textarea id="commentInput" rows="5" cols="50"
								placeholder="댓글을 입력하세요."></textarea>
							<button onclick="addComment()" id="btn">작성</button>
						</c:when>
						<c:otherwise>
							<h4>댓글 작성은 로그인 후 사용 가능합니다.</h4>
							<hr>
						</c:otherwise>
					</c:choose>
				</div>
				<div class="list_back_button"></div>
			</div>
		</div>
	</div>
	...
	<script>
						...
						.ajax({
							...
							success : function(comment) {
								...
								for (var i = 0; i < comment.length; i++) {
									...
									commentList
											.appendChild(createCommentElement(commentData));
								}
		...
		function createCommentElement(commentData) {
			...
			// 작성 시간, 작성자, 내용 추가
			commentElement.innerHTML = "<hr>" + "작성 시간: "
					+ commentData.comment_date + "<br> 작성자: "
					+ commentData.member_id + "<br> 내용: "
