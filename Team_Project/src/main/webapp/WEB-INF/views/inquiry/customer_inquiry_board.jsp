...
$(document).ready(function() {
    var memberNumberInputs = document.querySelectorAll('.member_number');
    var memberNumbers = [];
    memberNumberInputs.forEach(function(input) {
        var memberNumber = input.value;
        memberNumbers.push(memberNumber);
    });

    // 모든 게시글을 담아둘 변수
    var allInquiries = '';
    var session_member_number = $(".session_member_number").val(); // 세션에서 seller_number 가져오기
    // 체크박스 상태 변경 이벤트 핸들러
// 체크박스 상태 변경 이벤트 핸들러
$(".myPostsCheckbox").change(function() {
    if (this.checked) {
        ...
        $.ajax({
            type: "POST",
            url: "inquiry_my_only_member",
            data: {
                "member_numbers": memberNumbers,  // 판매자 번호의 배열을 전송
                "myPosts": true
            },
            success: function(response) {
                ...
                $(response).each(function(index, item) {
                    inquiry_my_only += `
                        <tr>
                        <input type="hidden" value="\${item.member_number}" class="member_number">
    ...
    $.ajax({
        type: "POST",
        url: "inquiry_all_member",// 모든 게시글을 불러올 엔드포인트 URL
        success: function(response) {
            var session_member_numbers = document.querySelectorAll('.session_member_number');
            var memberNumbers = [];
            // sellerNumbers 배열에 session_seller_numbers의 값을 추가
            session_member_numbers.forEach(function(input) {
                var session_member_number = input.value;
                memberNumbers.push(session_member_number);
            });

            $(response).each(function(index, item) {
                var isOwner = session_member_number==item.member_number;
                var buttonHtml = '';
...
<input type="hidden" value="${memberDTO.member_number }" class="session_member_number">
</tr>
...
<tr>
<input type="hidden" value="${i.member_number }" class="member_number">
